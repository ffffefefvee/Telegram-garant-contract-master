...
describe("CryptomusService.handleWebhook", () => {
  ...
  it("does not log webhook identifiers or signatures", async () => {
    const log = jest.spyOn(Logger.prototype, "log").mockImplementation();
    const service = makeService();
    const valid = sign(JSON.stringify(payload));

    await expect(
      service.handleWebhook({ ...payload, sign: valid }, valid),
    ).resolves.toBe(true);

    const logged = log.mock.calls
      .map(([message]) => String(message))
      .join("\n");
    expect(logged).not.toContain(payload.order_id);
    expect(logged).not.toContain(payload.uuid);
  ...
  it("rejects an invalid signature", async () => {
    const service = makeService();
    await expect(service.handleWebhook(payload, "deadbeef")).resolves.toBe(
      false,
    );
  });
