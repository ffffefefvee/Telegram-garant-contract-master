...
export interface TonLiteBlockLinkForward {
  ...
  destProof: Buffer;
  configProof: Buffer;
  signatures: TonLiteOrdinarySignatureSet;
}
...
function decodeLink(
  ...
  if (constructor === BLOCK_LINK_FORWARD_ID) {
    return {
      ...
      signatures: decodeSignatureSet(reader, limits.maxSignaturesPerLink),
    };
  }
...
export function verifyTonOrdinaryForwardLinkSignatures(
  ...
  const signedData = tonOrdinaryBlockSignedData(link.to);
