...
function parseSingleRoot(boc: Buffer, label: string): Cell {
  ...
  if (roots.length !== 1)
    throw new Error(`${label} must contain one root cell`);
  return roots[0];
}
...
export async function verifyCrossBuild(input: {
  ...
  if (blueprint.contract !== "TonNativeEscrow") {
  ...
  if (
    !blueprint.bocHex ||
    blueprintBoc.toString("hex") !== blueprint.bocHex.toLowerCase()
  ) {
  ...
  if (
    sha256(blueprintBoc) !==
    normalizeHash(blueprint.bocSha256, "Blueprint BOC SHA-256")
  ) {
  ...
  const manifest = {
    ...
    minOperationalReserveNano: blueprint.minOperationalReserveNano,
    toolchains: {
    ...
    artifacts: {
      blueprint: {
        bocSha256: sha256(blueprintBoc),
      },
      acton: {
        bocSha256: sha256(actonBoc),
      },
    },
    sources,
    approvals: {
...
async function main() {
  const blueprintPath = resolve(
    process.argv[2] ?? "build/cross/blueprint/TonNativeEscrow.compiled.json",
  );
  const actonPath = resolve(
    process.argv[3] ?? "build/cross/acton/TonNativeEscrow.json",
  );
  const outputPath = resolve(
    process.argv[4] ?? "build/TonNativeEscrow.release-candidate.json",
  );
  ...
  process.stdout.write(`Verified independent code hash ${manifest.codeHash}\n`);
  process.stdout.write(`Wrote unsigned release candidate ${outputPath}\n`);
}
